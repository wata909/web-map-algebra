//##############################################################################################################################
//アプリケーション変数
//##############################################################################################################################
const app	= {};
app.leaflet	= {
	'map'		: false,	//地図オブジェクト
	'controls'	: {},		//コントロールオブジェクト
	'layers'	: {			//レイヤーオブジェクト
		'ddmap'		: false
	},
	'markers'	: {			//クリック位置を示すマーカー
		'vertex'	: false,
		'message'	: false
	}
};
app.data	= {
	//評価条件
	'conditions'	: {},
	//総合評価
	'total'			: {
							'compile'	: { 'formula' : '', 'valid' : false },
							'judge'		: { 'formula' : '', 'valid' : false }
						}
};
app.extract	= {
	'center'	: false,
	'circle'	: false
};
app.mouse	= 0;
//画像保存
app.save	= {
	'points'	: [],		//頂点格納配列
	'rectangle'	: false		//LafletのRectangle要素
};
app.common	= {};

//******************************************************************************************************
//凡例等ポップアップ表示
const createLegendPopup	= function ( latlng_ ) {
	//******************************************************************
	//評価条件レイヤが表示中か否かを調べる
	const checked	= $( 'input.overlay:checked' ).val();
	//現在のズームレベルとクリック位置を取得
	const zoom	= app.leaflet.map.getZoom();
	const point	= app.leaflet.map.options.crs.latLngToPoint( latlng_, zoom ).round();
	//******************************************************************
	//テンプレートに埋め込むデータの変数を用意
	const parameters	= {};
	//promise配列の用意
	const promises	= [];
	//******************************************************************
	//評価レイヤからクリック位置の評価を取得するpromiseを作る
	const promise	= app.leaflet.layers.wine.export().then(
		function ( data_ ) {
			//canvas左上ポイント座標を計算
			const viewLT	= app.leaflet.map.getPixelBounds().min;
			const tileSize	= app.leaflet.layers.wine.getTileSize();
			const canvasLT	= viewLT.unscaleBy( tileSize ).floor().scaleBy( tileSize );
			//canvas上でのクリック位置
			const xy	= point.subtract( canvasLT );
			//ピクセル色取得
			const rgba	= data_.getContext( '2d' ).getImageData( xy.x, xy.y, 1, 1 ).data;
			//評価結果（不透明度）を登録する
			parameters.result	= {
				'value'	: ( rgba[3] / 255 ).toFixed( 2 )
			};
		}//function
	);//then
	//promiseを配列に格納
	promises.push( promise );
	//******************************************************************
	//評価条件レイヤ表示中の場合のみ次の処理
	if ( checked !== void(0) ) {
		//表示中の評価条件レイヤの定義を取得
		const target	= DEFINITIONS.conditions.filter(function ( value_ ) {
			return value_.key === checked;
		}).shift();
		//タイルサイズを取得し、クリック位置のタイル座標、タイル内座標を決める
		const tileSize	= app.leaflet.layers.ddmap.getTileSize();
		const tile	= point.unscaleBy( tileSize ).floor();
		const inner	= point.subtract( tile.scaleBy(tileSize) );
		tile.z	= zoom;
		//タイルのurlを決める
		const url	= app.leaflet.layers.ddmap.getTileUrl( tile );
		//**************************************************************
		//promise配列の用意
		const _promises	= [];
		//タイル取得のpromiseを登録
		_promises.push(
			utilities.loadImage( url )
		);
		//**************************************************************
		//表示中の評価条件レイヤがパレットタイルかつ凡例未取得の場合の処理
		if ( target.legend !== void(0) ) {
			if ( app.legend[checked] === void(0) ) {
				//凡例取得のpromiseを変数に格納
				app.legend[checked]	= $.ajax({
											'dataType'	: 'json',
											'url'		: target.legend
										});
			}
			_promises.push( app.legend[checked] );
		}//if
		//promiseの完了待ち処理
		const promise	= Promise.all( _promises ).then(
			function ( data_ ) {
				//引数配列の最初の要素がタイル、次の要素が凡例json
				const tile	= data_[0];
				const json	= data_[1];
				//テンプレートにはめる変数を用意
				parameters.condition	= {};
				//評価条件レイヤのタイトルは確定済み
				parameters.condition.title	= target.title;
				//タイル取得成否で分岐
				if ( tile === false ) {
					//取得失敗時はエラー
					parameters.condition.error	= 1;
				} else {
					//取得成功時
					//クリック位置のピクセルを取得
					app.click.canvas.width	= tile.width;
					app.click.canvas.height	= tile.height;
					const context	= app.click.canvas.getContext( '2d' );
					context.drawImage( tile, 0, 0 );
					const pixel	= context.getImageData( inner.x, inner.y, 1, 1 ).data;
					//凡例jsonの有無で分岐
					if ( json === void(0) ) {
						//凡例jsonがない場合：
						//値は24ビット符号付き整数を100で除算して計算
						const value	= (
										pixel[0] * 256 * 256 + pixel[1] * 256 + pixel[2]
										- ( pixel[0] < 128 ? 0 : Math.pow(256, 3) )
									) / 100;
						//パラメータに格納
						parameters.condition.value	= value;
						parameters.condition.unit	= target.unit;
						//テンプレートにパラメータをセット
						const template	= $.templates( '#popup_value' );
						const html	= template.render( parameters ).trim();
						//domをポップアップに追加
						app.click.popup.setContent( html );
					} else {
						//凡例jsonがある場合：
						//値は24ビット符号なし整数として計算
						const value	= pixel[0] * 256 * 256 + pixel[1] * 256 + pixel[2];
						//値を格納する
						parameters.condition.value	= value;
						//値を16進数に変換
						const value16	= (
											'000000'
											+ ( value ).toString( 16 )
										).slice( -6 );
						//16進数の値を凡例jsonと比較して該当する凡例を取得
						const legend	= json.items.filter(function ( value_ ) {
							return value_.value === value16;
						}).shift();
						//凡例取得成否で分岐
						if ( legend === void(0) ) {
							//取得失敗時はエラー
							parameters.condition.error	= 1;
						} else {
							//取得成功時
							//凡例をパラメータに追加
							parameters.condition.legend	= legend;
						}//if
					}//if
				}//if
			}//function
		);//then
		//promiseをpromise配列に登録
		promises.push( promise );
	}//if
	//promise配列の完了を待って処理
	Promise.all( promises ).then(
		function ( data_ ) {
			//表示に必要なパラメータはこの時点で作成済み
			//htmlを構築
			const html		= $.templates( '#popup' ).render( parameters );
			//ポップアップを表示
			app.click.popup
						.setLatLng( latlng_ )
						.openOn( app.leaflet.map )
						.setContent( html );
		}//function
	);//then
};//function
/* {/追記} */

//##############################################################################################################################
//メイン処理
//##############################################################################################################################
$( document ).ready(function () {
	app.common.saveMessageMarkerOption	= {
		'icon'	: new L.divIcon({
			'className'	: 'message',
			'html'		: $.templates( '#saveOrCancel' ).render({}),
			'iconSize'	: L.point( 240, 40 ),
		})	
	};
	app.common.clickMarkerOption	= {
		'icon'	: L.icon({
			'iconUrl'		: 'img/vertex.png',
			'iconsize'		: L.point( 15, 15 ),
			'iconAnchor'	: L.point( 8, 8 )
		})
	};
	app.common.circleOption	= {
		'color'			: "#E92D63",
		'weight'		: 3,
		'opacity'		: 0.8,
		'fillColor'		: "#562DE9",
		'fillOpacity'	: 0.4
	};
	app.common.rectangleOption	= {
		'color'			: "#E92D63",
		'weight'		: 3,
		'opacity'		: 0.8,
		'fillColor'		: "#562DE9",
		'fillOpacity'	: 0.4
	};

	//######################################################################################################
	//精度？の問題かIEだけ表示が違うので対象外とする（highp指定がNGかも）
	if ( L.Browser.ie ) {
		alert( 'Internet Explorer is not supported.' );
		return;
	}
	
	//######################################################################################################
	//フォーム関連

	//------------------------------------------------------------------------------
	//不透明度スライダ（すべて）の初期設定
	$( '.opacity .slider' ).slider({
		'min'	: 0,
		'max'	: 100,
		'step'	: 1,
		'value'	: DEFINITIONS.opacity
	});
	$( 'span', $('.opacity .slider').next() ).text( String(DEFINITIONS.opacity) );
	//評価レイヤのスライダ
	$( '#wine .opacity .slider' ).slider({
		'slide'	: function ( event_, ui_ ) {
						//スライダを動かした場合の処理
						//不透明度数値を書き換える
						$( 'span', $(this).next() ).text( String(ui_.value) );
						//レイヤの不透明度を変更する
						app.leaflet.layers.wine.setOpacity( ui_.value / 100 );
						//凡例の不透明度を変更する
						$( 'div.custom canvas.legend' ).css({ 'opacity' : ui_.value / 100 });
					}//function
	});//slider
	//DDマップレイヤのスライダ
	$( '#ddmap .opacity .slider' ).slider({
		'slide'	: function ( event_, ui_ ) {
						//スライダを動かした場合の処理
						//不透明度数値を書き換える
						$( 'span', $(this).next() ).text( String(ui_.value) );
						//レイヤの不透明度を変更する
						//DDマップが存在する場合のみ処理（スライダが操作可能な場合はDDマップあるはず）
						if ( app.leaflet.layers.ddmap !== void(0) ) {
							app.leaflet.layers.ddmap.setOpacity( ui_.value / 100 );
						}//
					}//function
	});//slider
	
	//------------------------------------------------------------------------------
	//背景地図の準備
	DEFINITIONS.basemap.forEach(function ( value_ ) {
		$( document.createElement('option') )
			.text( value_.title )
			.val( value_.template )
			.appendTo( $('#basemap') );
	});//forEach
	
	//------------------------------------------------------------------------------
	//評価条件をループして処理：formの作成とデータの準備
	DEFINITIONS.conditions.forEach(function ( value_ ) {
		//評価条件の組み立て
		const $li	= $( document.createElement('li') )
						.appendTo( $('ul.conditions') )
						.data({ 'type' : value_.key });

		/* {追記} */
		//評価条件レイヤをオーバーレイとして選択するためのラジオボタン
		const $label	= $( document.createElement('label') )
			.text( '表示' )
			.appendTo( $li );
		$( document.createElement('input') )
			.attr({ 'type' : 'radio', 'name' : 'overlay' })
			.val( value_.key )
			.addClass( 'overlay' )
			.prop( 'checked', false )
			.appendTo( $label );
		/* {/追記} */

		$( document.createElement('input') )
			.attr( 'type', 'checkbox' )
			.val( value_.key )
			.appendTo( $li );
		$( document.createTextNode(' ') ).appendTo( $li );
		$( document.createElement('input') )
			.addClass( 'formula' )
			.attr( 'type', 'button' )
			.val( '評価式' )
			.appendTo( $li );
		$( document.createTextNode(' ' + value_.title + '（' + value_.key + '）') ).appendTo( $li );
		
		//データの組み立て
		app.data.conditions[value_.key]	= {
			'template'	: value_.template,
			'formula'	: '',
			'type'		: value_.type,
			'samples'	: value_.samples,
			'valid'		: false
		};
	});//forEach

	//抽出条件フォーム
	$( '#inputExtract input[type="text"]' ).val( '' );

	//######################################################################################################
	//地図関連設定

	//******************************************************************************************************
	//地図の生成
	app.leaflet.map	= L.map( 'app', DEFINITIONS.mapOption );
	//zoomコントロールの位置変更
	app.leaflet.map.zoomControl.setPosition( 'bottomright' );
	app.leaflet.controls	= {};
	//スケールコントロールを追加
	app.leaflet.controls.scale		= L.control.scale({ 'position' : 'bottomleft' }).addTo( app.leaflet.map );
	//ズームレベル確認コントロールの追加
	app.leaflet.controls.showZoom	= L.Control.showZoom({ 'position' : 'topright' }).addTo( app.leaflet.map );
	//保存ボタンコントロールの追加
	app.leaflet.controls.buttons	= L.Control.buttons({ 'position' : 'topright' })
										.addTo( app.leaflet.map )
										.addClass( 'custom_buttons' )
										.addLabel( '取得する', 'ok' )
										.addLabel( '取り消す', 'ng' )
										.hide();
	//凡例コントロールの追加
	app.leaflet.controls.legend	= L.Control.legend({
														'position'	: 'bottomright',
														'color'		: [ 255, 0, 255, 255 ],
														'width'		: 30,
														'height'	: 119,
														'leader'	: 5
													}).addTo( app.leaflet.map );

	/* {追記} */
	//地図クリックに関連するオブジェクト
	app.click	= {
		'canvas'	: document.createElement( 'canvas' )
	};
	app.click.canvas.width	= 1;
	app.click.canvas.height	= 1;
	app.click.popup	= L.popup();
	//パレットタイルの凡例を格納するオブジェクト
	app.legend	= {};
	/* {/追記} */

	//凡例不透明度の設定
	$( 'div.custom canvas.legend' ).css({ 'opacity' : $('#wine .opacity .slider').slider('value') / 100 });
	
	//******************************************************************************************************
	//背景地図を登録する
	app.leaflet.layers.basemap	= L.tileLayer( $('#basemap').val(), {
		'attribution'	: '<a href="https://maps.gsi.go.jp/development/ichiran.html">地理院タイル</a>'
	}).addTo( app.leaflet.map );
	//背景地図変更時の処理
	$( '#basemap' ).change(function () {
		app.leaflet.layers.basemap.setUrl( $(this).val() );
	});//change
	
	//******************************************************************************************************
	//評価条件レイヤの追加
	app.leaflet.layers.wine	= ( new L.WebglLayer(app.data) )
									.addTo( app.leaflet.map )
									.setOpacity( $('#wine .opacity .slider').slider('value') / 100 );
	
	//######################################################################################################
	//評価条件関連処理

	//******************************************************************************************************
	//評価条件レイヤチェックボックスクリック時の処理
	$( 'ul.conditions li input[type="checkbox"]' ).click(function(){
		//classの追加と削除：適用時はapplyクラスをつける（表示の変更）
		$( this ).parent().toggleClass( 'apply' );
		//データの有効・無効を登録する
		const key	= $( this ).val();
		app.data.conditions[key].valid	= $( this ).prop( 'checked' );
		//データの有効・無効で分岐処理：ボタンの文字色変更
		if ( !app.data.conditions[key].valid ) {
			$( this ).next( 'input' ).removeClass( 'notready' );
		} else if ( !app.data.conditions[key].formula.length ) {
			$( this ).next( 'input' ).addClass( 'notready' );
		}
		
		//チェックボックスを操作したら評価基準式と評価判定式のボタン操作を必ず要求する
		app.data.total.compile.valid	= false;
		app.data.total.judge.valid		= false;
		//ボタンを赤文字にする
		$( 'div.total input' ).removeClass( 'notready' ).addClass( 'notready' );
		
	});//click
	//******************************************************************************************************
	//評価条件の評価式ボタンクリック時の処理
	$( 'ul.conditions li input[type="button"].formula' ).click(function () {
		//チェックボックスを取得しておく
		const $check	= $( this ).siblings( 'input[type="checkbox"]' );
		//チェックボックスのオンオフで分岐処理
		if ( $check.prop('checked') === false ) {
			//オフの場合
			$( '#valueFormulaError' ).text( '評価式を入力する前にチェックして条件を有効化してください。' );
			app.modals.valueFormulaError.dialog( 'open' );
		} else {
			//オンの場合
			//見出しを取得してモーダルウインドウのタイトルを設定する
			const title		= $( this ).parent().text().trim();
			app.modals.valueFormula.dialog({
				'title'	: '評価式の入力（' + title + '）'
			});
			//キーを取得する
			const key		= $check.val();
			//キーを使ってモーダルのtextareaに値をセットする
			$( 'textarea', app.modals.valueFormula )
				.val( app.data.conditions[key].formula )
				.data({ 'type' : key });
			//組み込みボタン以外に値のボタンを追加する
			const $values	= $( '#valueFormula div.values' );
			$values.empty();
			$( document.createElement('input') )
				.attr({ 'type' : 'button' })
				.addClass( 'insert' )
				.val( key )
				.data( 'text', key )
				.appendTo( $values );
			//組み込みボタンはすべて表示
			$( '#valueFormula div.buttons' ).show();
			
			//サンプルボタンを表示
			$( '#valueFormula div.buttons.samples input' ).show();
			//サンプルボタンで入力する数式をセット
			$( '#valueFormula div.buttons.samples input.sample' ).data( 'formula', app.data.conditions[key].samples[0] );
			//モーダルウインドウを開く
			app.modals.valueFormula.dialog( 'open' );
		}//if
	});//click
	//******************************************************************************************************
	//評価基準式、結果判定式ボタンクリック時の処理
	$( 'div.total input[type="button"]' ).click(function () {
		//採用する評価条件を格納する変数を用意
		const useConditions	= [];
		//式が空でなく有効化されている場合はそのキーを変数に格納
		for ( let i in app.data.conditions ) {
			//validはformulaが入力されている前提でtrueになるので後者のチェックだけで良いかもしれない
			if ( app.data.conditions[i].formula.length > 0 && app.data.conditions[i].valid ) {
				useConditions.push( i );
			}//if
		}//for
		//評価条件チェックと評価式の入力有無を確認
		if ( useConditions.length === 0 ) {
			//評価条件が正しく指定されていない
			$( '#valueFormulaError' ).text( $(this).val() + 'を入力する前に適用する評価条件を選択し、各条件の評価式を入力してください。' );
			app.modals.valueFormulaError.dialog( 'open' );
		} else {
			//評価条件が正しく指定されている
			const type	= $( this ).attr( 'name' );
			//モーダルのtextareaに値をセットする（既に一度入力されている場合はその値、初めての場合は空文字列）
			$( 'textarea', app.modals.valueFormula )
				.val( app.data.total[type].formula )
				.data({ 'type' : type });
			//モーダルのタイトルを設定する
			app.modals.valueFormula.dialog({
				'title'	: $( this ).val() + 'の入力'
			});
			//組み込みボタン以外に式で使用する各値のボタンを追加する
			const $values	= $( '#valueFormula div.values' );
			$values.empty();
			if ( type === 'compile' ) {
				//評価基準式の場合はチェックされた評価条件すべてのボタン
				useConditions.forEach(function ( value_ ) {
					$( document.createElement('input') )
						.attr({ 'type' : 'button' })
						.addClass( 'insert' )
						.val( value_ )
						.data( 'text', value_ )
						.appendTo( $values );
					$( document.createTextNode(' ') ).appendTo( $values );
				});
			} else {
				//結果判定式の場合はvalueボタン1つ
				$( document.createElement('input') )
					.attr({ 'type' : 'button' })
					.addClass( 'insert' )
					.val( 'value' )
					.data( 'text', 'value' )
					.appendTo( $values );
				//説明も加えておく
				$( document.createElement('span') ).css({ 'font-size' : '11px' }).text( '：評価基準式の値' ).appendTo( $values );
			}
			//サンプルボタンは非表示
			$( '#valueFormula div.buttons.samples input' ).hide();
			//モーダルを開く
			app.modals.valueFormula.dialog( 'open' );
		}
	});//click
	//******************************************************************************************************
	//反映ボタンクリック時の処理
	$( '#wine' ).submit(function() {
		if ( app.data.total.compile.valid && app.data.total.judge.valid ) {
			const obj	= new StringToFormula();
			const options	= $.extend( true, {}, app.data );
			for ( let key_ in options ) {
				for ( let key__ in options[key_] ) {
					if ( options[key_][key__].valid === void(0) || options[key_][key__].valid === true ) {
						const formula	= options[key_][key__].formula;
						const compiled	= obj.compile( formula, key__ );
						options[key_][key__].formula	= compiled;
					}
				}
			}
			const result	= app.leaflet.layers.wine.updateProgram( options );
			if ( result === false ) {
				$( '#valueFormulaError' ).text( '数式のコンパイルに失敗しました。各数式を確認してください。' );
				app.modals.valueFormulaError.dialog( 'open' );
			} else {
				//反映ボタンの文字色を変更する
				$( 'div.total input[type="submit"]' ).removeClass( 'notready' );
				
				/* {追記} */
				//ポップアップがある場合は表示を更新する
				if ( app.click.popup.isOpen() ) {
					const latlng	= app.click.popup.getLatLng();
					//GPUの処理完了を取得できないので0.5秒処理待ちする
					setTimeout(function () {
						createLegendPopup( latlng );
					}, 500 );
				}
				/* {/追記} */

			}//
		} else {
			$( '#valueFormulaError' ).text( '評価条件、評価基準式、結果判定式で確認が必要な項目があります。' );
			app.modals.valueFormulaError.dialog( 'open' );
		}
		return false;
	});//submit
	
	//######################################################################################################
	//数式エディタ
	
	//モーダルウインドウ処理
	app.modals	= {};
	//******************************************************************************************************
	//数式入力用モーダル
	app.modals.valueFormula	= $( '#valueFormula' ).dialog({
		'width'		: 340,
		'modal'		: true,
		'dialogClass'	: 'no-close',
		'buttons'	: [
							{
								'text'	: '取消',
								'icon'	: 'ui-icon-cancel',
								'click'	: function () {
												$( this ).dialog( 'close' );
											}//function
							},
							{
								'text'	: '適用',
								'icon'	: 'ui-icon-pencil',
								'click'	: function () {
												//------------------------------------------
												//数式の適用先を決める
												const type		= $( 'textarea', this ).data( 'type' );
												//モーダル内のキーボタンからキーの配列を作る（バリデーション準備）
												const keys	= [];
												$( 'div.buttons.values input[type="button"]', this ).each(function ( index_, element_ ) {
													keys.push( $(element_).val() );
												});
												//数式から不要なものを取り去る（空行や行前後の空白文字）
												const formula	= $( 'textarea', this ).val().split( "\n" ).reduce(function (acc_, cur_) {
													const line	= cur_.trim();
													if ( line.length > 0 ) {
														return acc_.concat( [line] );
													} else {
														return acc_;
													}
												}, [] ).join( "\n" );
												//バリデーションのため数式内のキーを特定文字列に置換
												const _formula	= keys.reduce(function (acc_, cur_) {
													const regex	= new RegExp( cur_, 'g' );
													return acc_.replace( regex, 'value' );
												}, formula );
												
												//------------------------------------------
												//バリデーション
												const obj	= new StringToFormula();
												if ( formula.length === 0 ) {
													//評価式は必須
													alert( '評価式は必須です。' );
												} else if ( !obj.validate(_formula) ) {
													//バリデーション失敗
													alert( '評価式に誤りがあります。' );
												} else {
													//適用先に応じて分岐処理
													if ( type !== 'compile' && type !== 'judge' ) {
														//評価条件用
														app.data.conditions[type].formula	= formula;
														$( 'ul.conditions input[value="' + type + '"]' ).next( 'input' ).removeClass( 'notready' );
													} else {
														//評価基準・結果判定用
														app.data.total[type].formula	= formula;
														app.data.total[type].valid		= true;
														$( 'div.total input[name="' + type + '"]' ).removeClass( 'notready' );
													}//
													$( this ).dialog( 'close' );
												}//
											}//function
							}
						],
		'autoOpen'	: false
	});//dialog
	//******************************************************************************************************
	//未チェック時注意モーダル
	app.modals.valueFormulaError	= $( '#valueFormulaError' ).dialog({
		'width'		: 340,
		'modal'		: true,
		'title'		: 'メッセージ',
		'dialogClass'	: 'no-close',
		'buttons'	: [
							{
								'text'	: '了解',
								'icon'	: 'ui-icon-info',
								'click'	: function () {
												$( this ).dialog( 'close' );
											}
							}
						],
		'autoOpen'	: false
	});//dialog
	//******************************************************************************************************
	//文字列挿入ボタンクリック時の処理
	$( '#valueFormula' ).on( 'click', 'input.insert', function () {
		//挿入文字列
		const insertStr	= $( this ).data( 'text' );
		//カーソル位置に文字列を挿入
		const $textarea	= $( '#valueFormula textarea' );
		const position	= $textarea.get( 0 ).selectionStart;
		const text		= $textarea.val();
		$textarea.val(
			text.substr( 0, position ) + insertStr + text.substr( position )
		);
		//textareaをfocus
		$textarea.get( 0 ).focus();
		//次のカーソル位置を決めてセットする
		const nextCursor	= $( this ).data( 'insert' );
		const afterPosition	= ( nextCursor === void(0) ? insertStr.length : parseInt($(this).data('insert'), 10) ) + position;
		$textarea.get( 0 ).setSelectionRange( afterPosition, afterPosition );
	});
	//******************************************************************************************************
	//サンプルボタンクリック時の処理
	$( '#valueFormula div.buttons.samples input.sample' ).click(function () {
		//ボタンに割り当てられた式を取得
		const sample	= $( this ).data( 'formula' );
		//textareaに式挿入
		const $textarea	= $( '#valueFormula textarea' );
		//textareaが空の場合はそのまま、入力があれば上書き確認の後式挿入
		if ( $textarea.val().length === 0 || confirm('既存の入力を上書きしますか。') ) {
			$textarea.val( sample );
		}
	});
	//******************************************************************************************************
	//画像保存における地図クリック処理
	app.leaflet.map.on( 'click', function ( event_ ) {
		//画像保存が開始されているか否か
		if ( app.mouse === 2 ) {
			//2点が保存されている場合のクリック（新たに1点目として登録するための準備）
			if ( app.save.points.length === 2 ) {
				//クリック地点を空にする
				app.save.points	= [];
				//表示中の図郭を削除する
				app.save.rectangle.remove();
				app.save.rectangle	= false;
				app.leaflet.markers.message.remove();
				
				app.leaflet.controls.buttons.hide();
				$( '#selectType' ).hide();
			}//if
			
			//登録されている点の数で分岐
			if ( app.save.points.length === 0 ) {
				//登録された点がない場合の処理（1点目のクリック）
				app.leaflet.markers.vertex	= L.marker( event_.latlng, app.common.clickMarkerOption )
												.addTo( app.leaflet.map );
			} else if ( app.save.points.length === 1 ) {
				//1点だけが登録されている場合の処理（2点目のクリック）
				app.leaflet.markers.vertex.remove();
				app.leaflet.markers.vertex	= false;
				const anchor	= app.save.rectangle.getCenter();
				app.leaflet.markers.message	= L.marker( anchor, app.common.saveMessageMarkerOption )
												.addTo( app.leaflet.map );
				app.leaflet.controls.buttons.show();
			}//if

			//クリック地点を登録する
			app.save.points.push( event_.latlng );
		}//if
	});
	//******************************************************************************************************
	//画像保存におけるマウス移動処理
	app.leaflet.map.on( 'mousemove', function ( event_ ) {
		if ( app.mouse === 2 ) {
			//1点だけが決まっているとき、マウス移動に追随して図郭を描画する
			if ( app.save.points.length === 1 ) {
				//図郭未描画の場合は描画する
				if ( app.save.rectangle === false ) {
					const area = [
						app.save.points[0],
						[app.save.points[0].lat, event_.latlng.lng],
						[event_.latlng.lat, app.save.points[0].lng],
						event_.latlng
					];
					app.save.rectangle = L.rectangle( area, app.common.rectangleOption )
											.addTo( app.leaflet.map );
				}//if
				//描画済みの図郭に対して、マウス移動で逐次座標を図郭に反映する
				app.save.rectangle.setBounds([
						app.save.points[0],
						[app.save.points[0].lat, event_.latlng.lng],
						[event_.latlng.lat, app.save.points[0].lng],
						event_.latlng
					]);
			}//if
		}//if
	});//function
	//******************************************************************************************************
	//保存開始ボタンのクリック
	$( 'ul.operation li.save input[name="start"]' ).click(function () {
		//保存実行フラグを有効にする
		app.mouse	= 2;
		//保存処理中の表示を出す
		$( '#saving' ).show();
		//操作パネルを無効化する
		$( '#navi input, #navi select' ).prop( 'disabled', true );
	});//click
	//******************************************************************************************************
	//表示領域全選択
	$( '#saving input.all' ).click(function () {
		//マーカー、矩形、メッセージがあればすべて一旦削除する
		if ( app.leaflet.markers.vertex !== false ) {
			app.leaflet.markers.vertex.remove();
			app.leaflet.markers.vertex	= false;
		}
		if ( app.save.rectangle !== false ) {
			app.save.rectangle.remove();
			app.save.rectangle	= false;
		}
		if ( app.leaflet.markers.message !== false ) {
			app.leaflet.markers.message.remove();
			app.leaflet.markers.message	= false;
		}

		//北西と南東の経緯度を取得して格納
		const bounds	= app.leaflet.map.getBounds();
		const nwLatLng	= bounds.getNorthWest();
		const seLatLng	= bounds.getSouthEast();
		app.save.points	= [ nwLatLng, seLatLng ];
		//矩形とメッセージを表示
		app.save.rectangle	= L.rectangle( bounds, app.common.rectangleOption )
								.addTo( app.leaflet.map );

		const anchor	= app.save.rectangle.getCenter();
		app.leaflet.markers.message	= L.marker( anchor, app.common.saveMessageMarkerOption )
										.addTo( app.leaflet.map );
		//ボタンを表示
		app.leaflet.controls.buttons.show();
	});//click
	//******************************************************************************************************
	//保存終了ボタンのクリック
	$( 'ul.operation li.save input[name="cancel"], #saving input.cancel' ).click(function () {
		//保存実行フラグを無効にする
		app.mouse	= 0;
		//取得ボタン取り消しボタンを隠す
		app.leaflet.controls.buttons.hide();
		//保存処理中の表示を隠す
		$( '#saving' ).hide();
		//マーカー、矩形、メッセージがあればすべて削除する
		if ( app.leaflet.markers.vertex !== false ) {
			app.leaflet.markers.vertex.remove();
			app.leaflet.markers.vertex	= false;
		}
		if ( app.save.rectangle ) {
			app.save.rectangle.remove();
			app.save.rectangle	= false;
		}
		if ( app.leaflet.markers.message ) {
			app.leaflet.markers.message.remove();
		}
		//保存種別を隠す
		$( '#selectType' ).hide();
		//フォームを有効化する
		$( '#navi input, #navi select' ).prop( 'disabled', false );
		//クリック地点を空にする
		app.save.points	= [];
	});//click
	//******************************************************************************************************
	//取り消すボタンのクリック：選択範囲をキャンセルしてボタンを隠す
	$( '.custom_buttons .ng' ).click(function () {
		//マーカー、矩形、メッセージがあればすべて削除する
		if ( app.leaflet.markers.vertex !== false ) {
			app.leaflet.markers.vertex.remove();
			app.leaflet.markers.vertex	= false;
		}
		if ( app.save.rectangle !== false ) {
			app.save.rectangle.remove();
			app.save.rectangle	= false;
		}
		if ( app.leaflet.markers.message ) {
			app.leaflet.markers.message.remove();
		}
		//保存種別を隠す
		$( '#selectType' ).hide();
		//クリック地点を空にする
		app.save.points	= [];
	});//click
	//******************************************************************************************************
	//取得するボタンのクリック：選択ボタンの表示
	$( '.custom_buttons .ok' ).click(function () {
		$( '#selectType' ).show();
	});//click
	//******************************************************************************************************
	//ファイル保存時のタイムスタンプ生成関数
	const createTimestamp	= function () {
		const date	= new Date();
		const values	= [];
		values.push( date.getFullYear() );
		values.push( date.getMonth() + 1 );
		values.push( date.getDate() );
		values.push( date.getHours() );
		values.push( date.getMinutes() );
		values.push( date.getSeconds() );
		return values.reduce(function (acc_, cur_) {
			const value	= String( cur_ );
			return acc_ + ( value.length === 1 ? '0' : '' ) + value;
		}, '' );
	};
	//******************************************************************************************************
	//ボタンクリックでダイアログを隠す
	$( '#selectType ul li' ).click(function () {
		$( '#selectType' ).hide();
	});
	//******************************************************************************************************
	//背景地図と評価結果レイヤーの保存
	$( '#selectType ul li.image' ).click(function () {
		//ズームレベル
		const zoom		= app.leaflet.map.getZoom();
		//保存範囲図郭を取得
		const saveBounds	= app.save.rectangle.getBounds();
		
		//背景地図のタイルサイズ
		const tileSize	= app.leaflet.layers.basemap.getTileSize();
		
		//保存範囲北西経緯度、南東経緯度
		const nwSaveBoundsLatLng	= saveBounds.getNorthWest();
		const seSaveBoundsLatLng	= saveBounds.getSouthEast();
		
		//画面表示範囲の北西と南東の経緯度を取得
		const displayBounds	= app.leaflet.map.getBounds();
		const nwDisplayBoundsLatLng	= displayBounds.getNorthWest();
		const seDisplayBoundsLatLng	= displayBounds.getSouthEast();
		
		//保存範囲全体が画面表示範囲に収まっているか確認
		if (
			nwSaveBoundsLatLng.lng < nwDisplayBoundsLatLng.lng
			|| seDisplayBoundsLatLng.lng < seSaveBoundsLatLng.lng
			|| nwDisplayBoundsLatLng.lat < nwSaveBoundsLatLng.lat
			|| seSaveBoundsLatLng.lat < seDisplayBoundsLatLng.lat
		) {
			//範囲に収まっていない場合はエラーを表示して終了
			$( '#valueFormulaError' ).text( '保存する範囲はすべてウインドウ内に表示されている必要があります。' );
			app.modals.valueFormulaError.dialog( 'open' );
			return false;
		}//if
		
		//保存範囲北西ピクセル座標、南東ピクセル座標
		const nwSaveBoundsPixel	= app.leaflet.map.options.crs.latLngToPoint( nwSaveBoundsLatLng, zoom ).round();
		const seSaveBoundsPixel	= app.leaflet.map.options.crs.latLngToPoint( seSaveBoundsLatLng, zoom ).round();
		
		//保存範囲における背景地図の北西タイル座標、南東タイル座標
		const nwTile	= nwSaveBoundsPixel.unscaleBy( tileSize ).floor();
		const seTile	= seSaveBoundsPixel.unscaleBy( tileSize ).floor();
		//保存範囲図郭北西端と保存範囲北西タイル左上とのピクセルのオフセット
		const offset	= nwSaveBoundsPixel.subtract( nwTile.scaleBy(tileSize) );
		
		//出力用canvas
		const canvas	= document.createElement( 'canvas' );
		canvas.width	= seSaveBoundsPixel.x - nwSaveBoundsPixel.x + 1;
		canvas.height	= seSaveBoundsPixel.y - nwSaveBoundsPixel.y + 1;
		const context	= canvas.getContext( '2d' );
		
		//背景地図のURLテンプレート
		const template	= app.leaflet.layers.basemap._url;
		//背景地図のタイル取得プロミス
		const promises	= [];
		for ( let j=nwTile.y; j<seTile.y+1; j++ ) {
			for ( let i=nwTile.x; i<seTile.x+1; i++ ) {
				const tile	= { 'x' : i, 'y' : j, 'z' : zoom };
				(function ( tile_ ) {
					const url	= L.Util.template( template, tile_ );
					promises.push(
						utilities.loadImage( url ).then(
							function ( data_ ) {
								if ( data_ !== false ) {
									context.drawImage(
														data_,
														( tile_.x - nwTile.x ) * tileSize.x - offset.x,
														( tile_.y - nwTile.y ) * tileSize.y - offset.y
													);
								}//if
							}//function
						)//then
					);//push
				})( tile );
			}//for
		}//for
		Promise.all( promises ).then(
			//data_は空
			function ( data_ ) {
				//この時点で背景canvasは完成している
				//WebGLレイヤを取得する（返り値がプロミス）
				return app.leaflet.layers.wine.export( saveBounds );
			}//function
		).then(
			function ( data_ ) {
				//不透明度を考慮してWebGLレイヤcanvasを貼り付け
				context.globalAlpha	= $( '#wine .opacity .slider').slider( 'value' ) / 100;
				context.drawImage( data_, 0, 0 );
				//ファイルをダウンロードさせる
				canvas.toBlob(function ( value_ ) {
					saveAs( value_, 'result_' + createTimestamp() + '.png' );
				});
			}//function
		);//then
	});
	//******************************************************************************************************
	//評価結果レイヤーとワールドファイルを保存
	$( '#selectType ul li.world' ).click(function () {
		//ファイル名を決める
		const filename	= 'result';
		//保存範囲図郭を取得
		const saveBounds	= app.save.rectangle.getBounds();
		app.leaflet.layers.wine.export( saveBounds ).then(
			function ( data_ ) {
				//zipインスタンス生成
				const zip	= new JSZip();
				//canvasからpngのbase64エンコード文字列を作る
				const base64	= data_.toDataURL( 'image/png' ).replace( 'data:image/png;base64,', '' );
				zip.file( filename + '.png', base64, { 'base64' : true });
				const coords	= [];
				const nwLatLng	= saveBounds.getNorthWest();
				const seLatLng	= saveBounds.getSouthEast();
				coords.push( L.Projection.SphericalMercator.project( nwLatLng ) );
				coords.push( L.Projection.SphericalMercator.project( seLatLng ) );
				const units		= {
					'x'	: Math.abs( coords[1].x - coords[0].x ) / data_.width,
					'y'	: Math.abs( coords[1].y - coords[0].y ) / data_.height * -1
				};
				const values	= [];
				values.push( String(units.x) );
				values.push( '0' );
				values.push( '0' );
				values.push( String(units.y) );
				values.push( String(coords[0].x + units.x / 2) );
				values.push( String(coords[0].y + units.y / 2) );
				zip.file( filename + '.pgw', values.join("\n") );
				return zip.generateAsync({ 'type' : 'blob' });
			}
		).then(
			function ( data_ ) {
				saveAs( data_, filename + '_' + createTimestamp() + '.zip' );
			}
		);
	});
	//******************************************************************************************************
	//抽出フォームのバリデーション関数
	const extractValidate	= function () {
		const validateValue	= function ( value_ ) {
			const value	= value_.trim();
			if ( value.length === 0 ) {
				return false;
			} else {
				const valueFloat	= parseFloat( value );
				if ( Number.isNaN(valueFloat) ) {
					return false;
				} else {
					return valueFloat;
				}
			}
		};
		const lat		= validateValue( $('#inputExtract input[name="latitude"]').val() );
		const lng		= validateValue( $('#inputExtract input[name="longitude"]').val() );
		const radius	= validateValue( $('#inputExtract input[name="radius"]').val() );
		if ( lat === false || lng === false || radius === false ) {
			return false;
		} else {
			return { 'lat' : lat, 'lng' : lng, 'radius' : radius };
		}
	};
	//******************************************************************************************************
	//抽出開始ボタンのクリック
	$( 'li.extract input[name="start"]' ).click(function () {
		//抽出フラグをセット
		app.mouse	= 1;
		//抽出フォームの位置を決めて表示
		const position	= $( this ).offset();
		const width		= $( this ).outerWidth();
		$( '#inputExtract' ).css({
			'top'	: String( position.top ) + 'px',
			'left'	: String( position.left + width + 2 ) + 'px'
		}).show();
		//抽出領域があれば描画する（以前入力した値の再表示）
		if ( app.extract.circle !== false ) {
			app.extract.circle.addTo( app.leaflet.map );
		}
		//操作パネルを無効化する
		$( '#navi input, #navi select' ).prop( 'disabled', true );
	});
	//******************************************************************************************************
	//抽出フォームの設定・解除ボタンクリック時の処理
	$( '#inputExtract input[type="button"]' ).click(function () {
		//フラグをセット
		app.mouse	= 0;
		//マーカーを隠す
		if ( app.leaflet.markers.vertex !== false ) {
			app.leaflet.markers.vertex.remove();
			app.leaflet.markers.vertex	= false;
		}
		//円を隠す（falseにするか否かはボタンで区別）
		if ( app.extract.circle !== false ) {
			app.extract.circle.remove();
		}
		//ボタンで分岐
		switch ( $(this).data('role') ) {
			//設定
			case 'set':
				//バリデーション
				const values	= extractValidate();
				if ( values === false ) {
					alert( '抽出条件を確認してください。' );
				} else {
					const latlng	= L.latLng({ 'lat' : values.lat, 'lng' : values.lng });
					app.leaflet.layers.wine
						.setExtractor( values.radius, latlng )
						.updateCanvas();
					$( '#navi input, #navi select' ).prop( 'disabled', false );
					$( '#inputExtract' ).hide();
				}
				break;
			//解除
			case 'cancel':
				//円を削除
				if ( app.extract.circle !== false ) {
					app.extract.circle	= false;
				}
				//フォームを空にする
				$( '#inputExtract input[type="text"]' ).val( '' );
				app.leaflet.layers.wine
					.setExtractor( 0 )
					.updateCanvas();
				$( '#navi input, #navi select' ).prop( 'disabled', false );
				$( '#inputExtract' ).hide();
				break;
			//設定・解除以外
			default:
				//ここは実行されない
		}//switch
	});
	//******************************************************************************************************
	//抽出フォーム入力中処理（リアルタイムで円を描画）
	$( '#inputExtract input[type="text"]' ).on( 'input', function () {
		//値のバリデーション
		const values	= extractValidate();
		if ( values === false ) {
			//値がおかしい場合は何もしない
		} else {
			//値が正しい場合
			//一旦円を削除する
			if ( app.extract.circle !== false ) {
				app.extract.circle.remove();
				app.extract.circle	= false;
			}
			//円を描く
			app.extract.center	= L.latLng({ 'lat' : values.lat, 'lng' : values.lng });
			app.extract.circle	= L.circle( app.extract.center, values.radius, app.common.circleOption )
									.addTo( app.leaflet.map );
			app.extract.center	= false;
		}
	});
	//******************************************************************************************************
	//抽出フラグ中のクリック処理
	app.leaflet.map.on( 'click', function ( event_ ) {
		//抽出フラグが立っている場合のみ処理
		if ( app.mouse === 1 ) {
			if ( app.extract.center === false ) {
				//フォームに経緯度を入力
				$( '#inputExtract input[name="latitude"]' ).val( event_.latlng.lat.toFixed(6) );
				$( '#inputExtract input[name="longitude"]' ).val( event_.latlng.lng.toFixed(6) );
				//中心がセットされていない場合（1回目のクリック）
				//円があれば削除
				if ( app.extract.circle !== false ) {
					app.extract.circle.remove();
					app.extract.circle	= false;
				}
				//中心をセット
				app.extract.center	= event_.latlng;
				app.leaflet.markers.vertex	= L.marker( event_.latlng, app.common.clickMarkerOption )
												.addTo( app.leaflet.map );
			} else {
				//中心がセットされている場合（2回目のクリック）
				//円の半径を求めて描画
				const radius	= app.leaflet.map.options.crs.distance( app.extract.center, event_.latlng );
				app.extract.circle.setRadius( radius );
				app.extract.center = false;
				app.leaflet.markers.vertex.remove();
			}//if
		}//if
	});//on
	//******************************************************************************************************
	//抽出フラグ中のマウス移動処理
	app.leaflet.map.on( 'mousemove', function ( event_ ) {
		//抽出フラグが立っている場合のみ処理
		if ( app.mouse === 1 ) {
			//中心がセットされている場合のみ処理
			if ( app.extract.center !== false ) {
				//半径を求める
				const radius	= app.leaflet.map.options.crs.distance( app.extract.center, event_.latlng );
				//半径を入力する
				$( '#inputExtract input[name="radius"]' ).val( radius.toFixed(0) );
				//円の有無で分岐
				if ( app.extract.circle === false ) {
					//円がなければ追加する
					app.extract.circle	= L.circle( app.extract.center, radius, app.common.circleOption )
											.addTo( app.leaflet.map );
				} else {
					//円があれば半径を変更する
					app.extract.circle.setRadius( radius );
				}//if
			}//if
		}//if
	});//on

	//######################################################################################################
	//DDマップ関連処理
	//不透明度変更はスライダー生成時処理で記述、DDマップ追加処理はddmap.jsで記述
	//削除
	$( '#ddmap .opacity input.remove' ).click(function () {
		app.leaflet.layers.ddmap.remove();
		app.leaflet.layers.ddmap	= false;
		$( '#ddmap .opacity .slider' ).slider({ 'value' : DEFINITIONS.opacity });
		$( '#ddmap .opacity .value span' ).text( String(DEFINITIONS.opacity) );
		$( '#ddmap' ).hide();

		/* {追記} */
		//DDマップ（オーバーレイ）削除時は評価条件レイヤ表示ラジオボタンのチェックを外す
		$( 'input.overlay' ).prop( 'checked', false );
		//ポップアップがある場合は表示を更新する
		if ( app.click.popup.isOpen() ) {
			const latlng	= app.click.popup.getLatLng();
			createLegendPopup( latlng );
		}
		/* {/追記} */
	});

	//******************************************************************************************************
	/* {追記} */
	//評価条件のラジオボタンクリック（チェック）時の処理
	$( 'input.overlay' ).on( 'click', function () {
		//キーを受け取り、対象となるレイヤー情報を取得
		const key	= $( this ).val();
		const target	= DEFINITIONS.conditions.filter(function ( value_ ) {
			return value_.key === key;
		}).shift();
		//パネルのDDマップ欄表示
		$( '#ddmap' ).show();
		//titleの表示
		$( '#ddmap .title span' ).text( target.title );
		//DDマップ表示有無で分岐処理
		if ( app.leaflet.layers.ddmap !== false ) {
			//DDマップ表示あり
			//url更新のみ
			app.leaflet.layers.ddmap.setUrl( target.template );
		} else {
			//DDマップ表示なし
			//レイヤを生成して追加
			app.leaflet.layers.ddmap	= L.tileLayer( target.template, {
				'opacity'	: parseInt( $('#ddmap .opacity .slider').slider('value'), 10 ) / 100
			}).addTo( app.leaflet.map );
		}//if
		//評価レイヤーは常に最前面
		app.leaflet.layers.wine.bringToFront();
		
		//ポップアップがある場合は表示を更新する
		if ( app.click.popup.isOpen() ) {
			const latlng	= app.click.popup.getLatLng();
			createLegendPopup( latlng );
		}
	});
	//******************************************************************************************************
	//地図クリック時の処理
	app.leaflet.map.on( 'click', function ( event_ ) {
		//画像保存が開始されているか否か
		if ( app.mouse === 0 ) {
			createLegendPopup( event_.latlng );
		}
	});
});
